    ...
    public function obtenerCuentasXCobrarDetallado($cliente = null, $vendedor = null)
    {
        $query = DB::connection('factura')
            ...
            ->select(
                ...
                'dcc.fec_emis',
                DB::raw("CASE
                            WHEN (SELECT campo3 FROM factura WHERE fact_num = dcc.nro_doc) = \'\'
                ...
                DB::raw('DATEADD(DD, DATEDIFF(DD, dcc.fec_emis, dcc.fec_venc), CONVERT(SMALLDATETIME, (CASE WHEN dcc.tipo_doc = \'FACT\' THEN
                            CASE WHEN (SELECT campo3 FROM factura WHERE fact_num = dcc.nro_doc) = \'\'
                            THEN CONVERT(CHAR(10), dcc.fec_venc, 103)
                            ELSE (SELECT CONVERT(CHAR(10), campo3, 121) FROM factura WHERE fact_num = dcc.nro_doc)
                            END ELSE CONVERT(CHAR(10), dcc.fec_venc, 103) END), 103)) AS fec_venc'),
                'dcc.observa',
                'dcc.tasa',
                'dcc.moneda',
